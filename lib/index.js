...
CodeGenerator.prototype.generateModel = function(type, templateData, destination, cb) {
  fs.readFile(__dirname + '/templates/' + type + '.jade', 'utf-8', function(err, template) {
    ...
    fs.writeFile(destination + '/' + templateData.name + '.java', templateFunction(templateData), cb);
  });
}
...
CodeGenerator.prototype.generateContentProvider = function(destination, templateData, cb) {
  ...
  stewardess(
    ...
    function(next) {
      var databaseAndProviderDone = waitress(2, next);

      addHelpers(templateData, function() {
        fs.writeFile(destinationPackage + '/database/Database.java', templateMap['database'](templateData), databaseAndProviderDone);
        databaseAndProviderDone();
        //fs.writeFile(destinationPackage + '/provider/Provider.java', templateMap['provider'](templateData), databaseAndProviderDone);
      });
    }
  ).run();
}
