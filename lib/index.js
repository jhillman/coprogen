  ...
  async.series([
        ...
        return function(next) {
          ...
          addHelpers(model, function() {
            ...
            fs.writeFile(destinationPackage + '/database/table/' + model.name + 'Table.java', templateMap['database-table'](model), function() {
              writeFileWithPersistedSection(destinationPackage + '/model/' + model.name + '.java', templateMap['model'], model, next, cb);
            });
          });          
        }
      }), function() {
    ...
    function(next) {
      addHelpers(contentProviderData, function() {
        async.parallel([
          function(next) {
            writeFileWithPersistedSection(destinationPackage + '/database/' + helpers.capitalize(helpers.camelCase(contentProviderData.databaseName)) + 'Database.java', templateMap['database'], contentProviderData, next, cb);
          },
          function(next) {
